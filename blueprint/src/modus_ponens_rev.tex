...
\leanok
\uses{def:Z2^n}
For $m\in\mathbb{N}$, let $g : \Fin (2^m) \to \Zz^m$ be a bijective function. $g$ exists, {\B applying} the fact that $\Fin (2^m)$ and $\Zz^m$ have {\B equal cardinals}.
\end{definition}
...
\leanok
\uses{def:e}
If $i \in \Fin n$ and $f : \Fin n \to \Zz^m$, then
$\displaystyle\sum_{j \in \Fin n} e_{ij} \boldsymbol{\cdot} f(j) = f(i).$
\end{lemma}
