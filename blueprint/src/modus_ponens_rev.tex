...
\begin{proof}
Unfolding \eqref{e}, $e_{ij} = 1$ if $i = j$ and $e_{ij} = 0$ otherwise. We {\B rewrite}  $\sum_{j \in \Fin n} e_{ij} \boldsymbol{\cdot} f(j)$ as $f(i)$, using the fact that the {\B single} term that contributes in the {\B sum} is the one with $j = i$.
\end{proof}
...
First, we {\B rewrite} $n$ as $2^m$ in \eqref{SF}. {\B Let} $g$ be given by {\B Definition} \ref{bij_Fin} and {\B use} $f$ given by
