...
\begin{proof}
By the definition of \eqref{e}, $e_{ij} = 1$ if $i = j$ and $e_{ij} = 0$ otherwise. We {\B rewrite}  $\sum_{j \in \Fin n} e_{ij} \boldsymbol{\cdot} f(j)$ as $f(i)$, using the fact that the {\B single} term that contributes in the {\B sum} is the one with $j = i$.
\end{proof}
...
$$f(b) := g^{-1}\left(\sum_{j\in\Fin 2^m} b_j \boldsymbol{\cdot} g(j)\right).$$

{\B Introduce} $b\in\Zz^n$ and $k\in\Fin n$ in order to verify \eqref{SF} for $f$. We want to show
