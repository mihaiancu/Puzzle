...
\leanok
\uses{def:SF, def:d}
If $f:\Zz^n \to \Fin n$ is \eqref{SF}, $k\in\Fin n$, and $b\in\Zz^n$, then
$$\sum_{i\in\Fin n} \delta(f(b+{\rm e}_i),k) = 1.$$
\end{lemma}

\begin{proof}
\uses{fun_shift_inj}
In view of the defintion of \eqref{SF} for $f$, we {\B obtain} $i_k\in\Fin n$ such that $f(b+{\rm e}_{i_k}) = k$.

We shall {\B rewrite} the sum by proving that {\B only the $i_k$ term in the sum is equal to $1$, and the rest are $0$}.

Clearly, $\delta(f(b+{\rm e}_{i_k}),k) = 1$, by the definition of \eqref{d} and the logic of {\B if-then-else}.

{\B Introduce} $i\in\Fin n$ with $i\neq i_k$ and consider the definition of \eqref{d}. {\B Rewritting} $k$ as  $f(b+{\rm e}_{i_k})$ and {\B applying} the logic of {\B if-then-else} in \eqref{d}, we have to prove that $f(b+{\rm e}_i) \neq f(b+{\rm e}_{i_k})$. In order to get a {\B contradiction}, assume that $f(b+{\rm e}_i) = f(b+{\rm e}_{i_k})$. Then, by {\B Lemma} \ref{fun_shift_inj}, we {\B have} $i=i_k$, which is {\B false} in view of the assumption $i\neq i_k$.
\end{proof}
